    ...
    <div className="space-y-6">
      ...
      >
        <div className={cn(
          ...
          isInputFocused
            ? "bg-white/80 dark:bg-slate-900/80 ring-2 ring-success/50 shadow-[0_0_40px_rgba(16,185,129,0.2)]"
          ...
          <form onSubmit={handleSearch} className="p-2 flex flex-col md:flex-row gap-2 relative z-10">
            ...
            <div className="h-px md:h-14 w-full md:w-px bg-surface-200 dark:bg-white/5 flex items-center justify-center">
              <button
                type="button"
                onClick={handleSwap}
                className="p-2 rounded-full bg-black/5 dark:bg-white/5 hover:bg-black/10 hover:dark:bg-white/10 text-slate-400 hover:text-surface-900 hover:dark:text-white transition-all transform hover:rotate-180 border border-surface-200 dark:border-white/5"
            ...
            <div className="w-full md:w-40 relative group">
              ...
              >
                {[3, 4, 5, 6, 7, 8].map((h) => (
                  <option key={h} value={h} className="bg-white dark:bg-slate-900 text-surface-900 dark:text-white">
                    {h} Hops
                  </option>
                ))}
              </select>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mt-4 text-xs text-slate-500">
          ...
          {EXAMPLE_PATHS.map((example) => (
            <button
              ...
              className="px-2 py-1 rounded-md hover:bg-surface-100 hover:dark:bg-white/5 hover:text-success-300 transition-colors font-mono"
            >
              {example.label}
            </button>
          ))}
            ...
            {result.found && result.packages.length > 0 && (
              <div className="glass-card p-6">
                ...
                <div className="relative py-8">
                  ...
                  <div className="absolute left-8 top-12 bottom-12 w-1 bg-surface-200 dark:bg-white/5 rounded-full overflow-hidden">
                        ...
                        >
                          ...
                          <div className="relative z-10 w-16 flex justify-center">
                            <motion.div
                              ...
                              className={cn(
                                "w-12 h-12 rounded-2xl flex items-center justify-center font-black shadow-xl border-2 transition-transform group-hover:scale-110",
                                isStart ? "bg-success border-success/20 text-white" :
                                  isEnd ? "bg-blue-500 border-blue-500/20 text-white" :
                                    "bg-white dark:bg-slate-900 border-surface-200 dark:border-white/10 text-surface-900 dark:text-white"
                              )}
                              style={{
                              ...
                              className={cn(
                                ...
                                isStart ? "bg-success/5 border-success/20 hover:bg-success/10" :
                                  isEnd ? "bg-blue-500/5 border-blue-500/20 hover:bg-blue-500/10" :
                                    "bg-white dark:bg-surface-900/60 border-surface-200 dark:border-white/5 hover:bg-surface-50 hover:dark:bg-surface-900/80 hover:border-surface-300 hover:dark:border-white/10"
                              )}
                            >
                              {/* Hover Gradient */}
                              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 dark:via-white/5 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700 pointer-events-none" />

                              <div className="flex items-center gap-4 relative z-10">
                                <div
                                  className="p-3 rounded-xl transition-transform group-hover:scale-110 shadow-lg animate-fade-in"
                                  style={{
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-3 mb-1">
                                    <h4 className={cn(
                                      "font-bold text-lg",
                                      isStart ? "text-success-600 dark:text-success-400" : isEnd ? "text-blue-600 dark:text-blue-400" : "theme-text-primary"
                                    )}>
                                      {pkg.name}
                                    </h4>
                                    <div className={cn(
                                      "px-2.5 py-0.5 rounded-md text-[10px] font-black uppercase tracking-widest border border-surface-200 dark:border-white/10",
                                      getEcosystemBadgeClass(pkg.ecosystem)
                                    )}>
                                      {formatEcosystemName(pkg.ecosystem)}
                                    </div>
                                  </div>
                                  <p className="text-xs font-mono tracking-wide theme-pill inline-block px-2 py-1 rounded-md">
                                    {pkg.id}
                                  </p>
                                </div>
                                ...
                                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                                  <button
                                    ...
                                    className="p-2.5 rounded-xl bg-surface-100 dark:bg-white/5 hover:bg-surface-200 hover:dark:bg-white/10 text-slate-500 dark:text-slate-400 hover:text-surface-900 hover:dark:text-white border border-surface-200 dark:border-white/5 hover:border-surface-300 hover:dark:border-white/10 transition-colors shadow-lg"
                                  ...
                                  <button
                                    ...
                                    className="p-2.5 rounded-xl bg-surface-100 dark:bg-white/5 hover:bg-danger/10 text-slate-500 dark:text-slate-400 hover:text-danger border border-surface-200 dark:border-white/5 hover:border-danger/20 transition-colors shadow-lg"
          ...
          >
            ...
            <div className="relative z-10 max-w-lg mx-auto">
              <div className="group/icon relative w-28 h-28 mx-auto mb-8">
                ...
                <div className="relative w-28 h-28 rounded-[2rem] bg-white dark:bg-slate-900 border border-surface-200 dark:border-white/10 flex items-center justify-center shadow-2xl group-hover/icon:-rotate-12 transition-all duration-500">
              ...
              <p className="text-lg theme-text-tertiary leading-relaxed mb-8">
                System ready. Enter source and target packages to calculate the shortest dependency path.
              </p>

              <div className="inline-flex items-center gap-3 px-6 py-3 rounded-full bg-success/5 border border-success/20 text-success-600 dark:text-success-300 font-bold uppercase tracking-widest text-xs animate-pulse">
