 ...
 *
 * Tests the human workflow on /graph with deterministic GraphQL route mocking (WMCP-2C, Section 68-74).
 */

import { test, expect } from "@playwright/test";

test.describe("War Room Human UI Graph Workflow (WMCP-2C)", () => {
  ...
  test("1. Initial ?pkg= URL opens graph through War Room actions to GRAPH_READY", async ({ page }) => {
    ...
    // Verify canonical state container attributes
  ...
  test("3. Human graph race: newer submitted graph completes and remains visible", async ({ page }) => {
    ...
    // npm:react completes and canonical root is npm:react
    const graphContainer = page.locator("[data-war-room-phase]");
    await expect(graphContainer).toHaveAttribute("data-war-room-root-package", "npm:react", { timeout: 15000 });
    await expect(graphContainer).toHaveAttribute("data-war-room-phase", "GRAPH_READY", { timeout: 15000 });
  });
