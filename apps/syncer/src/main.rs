...
async fn flush_graph_batch_inner(sink: &MemgraphSink, batch: &GraphBatch) -> Result<()> {
    // Upsert packages
...
async fn flush_vector_batch_inner(sink: &QdrantSink, batch: &VectorBatch) -> Result<()> {
    ...
    for emb in &batch.embeddings {
        ...
        sink.upsert_vector(&emb.package_id, emb.embedding.clone(), metadata)
            .await?;
    }

    info!(embeddings = batch.embeddings.len(), "Flushed vector batch");
