...
pub async fn handle_package_deleted(
    client: &MemgraphClient,
    payload: &[u8],
) -> Result<()> {
    // Decode protobuf message
    let event = PackageDeleted::decode(payload)
        .context("Failed to decode PackageDeleted protobuf")?;
    ...
    let query = GraphQueries::package_deleted(
        &event.ecosystem,
        &event.package_name,
    );
