...
use seed::{SeedConfig, SeedRunner};
...
async fn run_publish_mode(config: AppConfig, pool: sqlx::PgPool) -> Result<()> {
    ...
    let count: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM ingestion_outbox WHERE status = 'pending'")
            .fetch_one(&pool)
            .await
            .unwrap_or((0,));
...
async fn run_live_mode(cli: &Cli, config: AppConfig, pool: sqlx::PgPool) -> Result<()> {
    ...
    let producer = Arc::new(producer::EventProducer::new(
    ...
    let rotator = Arc::new(http::ProxyRotator::new(
        config.crawler.proxy_urls.clone(),
        &config.crawler,
    )?);

    // Start NPM if enabled
    let npm_enabled =
        matches!(cli.registry, Registry::Npm | Registry::All) && config.registries.npm_enabled;
    if npm_enabled {
        spawn_npm_ingestion(
            ...
            rotator.clone(),
            &config.kafka.brokers,
            Duration::from_secs(config.registries.poll_interval_secs),
        );
    }

    // Start PyPI if enabled
    let pypi_enabled =
        matches!(cli.registry, Registry::Pypi | Registry::All) && config.registries.pypi_enabled;
    if pypi_enabled {
    ...
    let cargo_enabled =
        matches!(cli.registry, Registry::Cargo | Registry::All) && config.registries.cargo_enabled;
    if cargo_enabled {
...
fn spawn_npm_ingestion(
    ...
    tokio::spawn(async move {
        loop {
            match NpmWatcher::new(
                watcher_store.clone(),
                watcher_producer.clone(),
                "raw.npm.changes.v1".to_string(),
                poll_interval,
            ) {
                Ok(watcher) => {
        ...
        loop {
            match EventConsumer::new(&brokers, "ingestion-npm-worker-v1", "raw.npm.changes.v1") {
                Ok(consumer) => {
                    ...
                    let worker =
                        NpmWorker::new(consumer, fetcher, state_store, worker_pool.clone());
                ...
                Err(e) => {
                    error!(
                        "Failed to create NPM Worker Consumer: {}. Retrying in 5s...",
                        e
                    );
                }
            }
...
fn spawn_pypi_ingestion(
    ...
    use registries::pypi::{PypiFetcher, PypiStateStore, PypiWatcher, PypiWorker};
    ...
    tokio::spawn(async move {
        loop {
            match PypiWatcher::new(
                watcher_store.clone(),
                watcher_producer.clone(),
                "raw.pypi.journal.v1".to_string(),
            ) {
                Ok(watcher) => {
                    if let Err(e) = watcher.run().await {
                ...
                Err(e) => {
                    error!(
                        "Failed to initialize PyPI Watcher: {}. Retrying in 5s...",
                        e
                    );
                }
            }
    ...
    tokio::spawn(async move {
        loop {
            match EventConsumer::new(&brokers, "ingestion-pypi-worker-v1", "raw.pypi.journal.v1") {
                ...
                Err(e) => {
                    error!(
                        "Failed to create PyPI Worker Consumer: {}. Retrying in 5s...",
                        e
                    );
                }
            }
    ...
    tokio::spawn(async move {
        loop {
            ...
            match producer {
                Ok(prod) => {
                    match CargoWatcher::new(prod) {
                        Ok(watcher) => {
                            ...
                            watcher
                                .watch_crates(&["tokio", "serde", "axum", "anyhow", "thiserror"])
                                .await;
                        ...
                        Err(e) => {
                            error!(
                                "Failed to initialize Cargo Watcher: {}. Retrying in 5s...",
                                e
                            );
                        }
                    }
                ...
                Err(e) => {
                    error!(
                        "Failed to create Kafka producer for Cargo: {:?}. Retrying in 5s...",
                        e
                    );
                }
            }
