...
- **WMCP-8 version-aware exposure:** REAL for direct dependency requirements; exposed, blocked, and unknown counts are reported separately. The final War Room UI labels the available count as direct dependents; no unproven transitive metric is claimed.
...
- **WMCP-11 migration planning:** REAL deterministic plan generation from review, WMCP-8 exposure, critical paths, and target-level breaking findings; OSV evidence remains a separate WMCP-9 axis and is not composed into the plan.
