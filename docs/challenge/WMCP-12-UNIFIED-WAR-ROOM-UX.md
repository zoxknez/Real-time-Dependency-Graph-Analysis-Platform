...
The `/graph` route is the challenge surface. Its status panel keeps API breaking findings, declared version exposure, direct dependents, live evidence, human review, critical paths, and migration plans visibly separate. No transitive reachability number is presented without a dedicated authoritative source. The UI states: `Declared version exposure does not prove downstream source incompatibility.`
