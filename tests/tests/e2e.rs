...
async fn test_qdrant_vector_operations() {
    ...
    let point = PointStruct::new("npm/lodash".to_string(), vec![0.1f32; 384], payload);

    client
        .upsert_points(UpsertPointsBuilder::new("test_packages", vec![point]))
        .await
        .unwrap();
    ...
    let results = client
        .search_points(
            SearchPointsBuilder::new("test_packages", vec![0.1f32; 384], 10).with_payload(true),
        )
        .await
        .unwrap();
...
async fn test_kafka_message_flow() {
    ...
    helpers::kafka::create_topic(&admin, &topic_name)
        .await
        .unwrap();
    ...
    helpers::kafka::send_message(&producer, &topic_name, &pkg.id, &payload)
        .await
        .unwrap();
    ...
    let consumer =
        helpers::kafka::create_consumer(&brokers, &format!("test-group-{}", unique_test_id()))
            .unwrap();
...
async fn test_full_e2e_flow() {
    ...
    qdrant_client
        .upsert_points(UpsertPointsBuilder::new("packages", vec![point]))
        .await
        .unwrap();
    ...
    let search_results = qdrant_client
        .search_points(
            SearchPointsBuilder::new("packages", vec![0.5f32; 384], 10).with_payload(true),
        )
        .await
        .unwrap();
...
async fn test_impact_radius_calculation() {
    ...
    let mut result = graph
        .execute(neo4rs::Query::new(
         ...
         RETURN count(DISTINCT d) as affected_count"
                .to_string(),
        ))
        .await
        .unwrap();
